...
function custom_styles_and_scripts()
{
    ...
    wp_enqueue_script('js-slick', get_stylesheet_directory_uri() . '/build/js/vendor/slick.min.js', array(), null, true);
    ...
    if (in_array('single-cpt-products', $classes)) {
        wp_enqueue_script('js-venobox', get_stylesheet_directory_uri() . '/build/js/vendor/venobox.min.js', array(), null, true);
    }

    if (in_array('single-cpt-books', $classes)) {
        wp_enqueue_script('js-venobox', get_stylesheet_directory_uri() . '/build/js/vendor/venobox.min.js', array(), null, true);
    }
...
{
    wp_enqueue_style('custom-admin', get_template_directory_uri() . '/build/css/admin.css');
}

add_action('admin_enqueue_scripts', 'custom_admin_styles_and_scripts');

/**
 * Include Backend / Gutenberg styling scripts
 */
function custom_add_gutenberg_to_admin()
{
    wp_register_style('custom_gutenberg_css', get_stylesheet_directory_uri() . '/build/css/gutenberg.css', false, '1.0.0');
    wp_enqueue_style('custom_gutenberg_css');
}

add_action('enqueue_block_editor_assets', 'custom_add_gutenberg_to_admin');
