...
// Add new role
add_role('studiopanadda', __('Angelos User', 'studiopanadda'), $capabilities);
...
add_filter('users_list_table_query_args', 'edit_users_list');

//Removing admin view from list/will be empty anyways
function edit_users_views( $views ) {
    if( !role_is_admin() ){
...
add_filter('views_users', 'edit_users_views');

// Remove administrator from all user-roles lists when user is not admin
function edit_user_roles( $roles ) {
    if( !role_is_admin() ){
...
add_filter('editable_roles', 'edit_user_roles');

// Prevent io-user of viewing edit-user.php for admin-id's
function check_user_has_right_to_edit_user() {
    global $pagenow;
    ...
    if( $pagenow == 'user-edit.php' ){
        ...
        if( $user_id != $edit_user_id) { //If not your own page!
            ...
            if( !role_is_admin() && in_array( 'administrator', $user_roles ) ){
                wp_die( __( 'Sorry, you are not allowed to edit this user.' ) );
            }
        }
    }
}
add_action('wp_loaded', 'check_user_has_right_to_edit_user');
